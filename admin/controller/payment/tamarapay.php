...
class ControllerPaymentTamarapay extends Controller {
    ...
    public function index() {
        ...
        $this->response->setOutput($this->load->view('payment/tamarapay.tpl', $this->getDataForIndexPage()));
    }
    ...
    protected function prepareLayoutDataForIndexPage(&$data) {
        ...
        $data['breadcrumbs'][] = [
            ...
            'href' => $this->url->link('payment/tamarapay', 'token=' . $this->session->data['token'], true)
        ];

        $data['action'] = $this->url->link('payment/tamarapay', 'token=' . $this->session->data['token'], true);
    ...
    protected function prepareVersionMessageForIndexPage(&$data) {
        $githubVersionLink = "https://raw.githubusercontent.com/tamara-solution/opencart/v20x/VERSION.txt";
        $githubVersion = @file_get_contents($githubVersionLink);
        $data['is_using_latest_version'] = null;
        if ($githubVersion) {
            $downloadLink = "https://github.com/tamara-solution/opencart/archive/refs/heads/v20x.zip";
            $readmeLink = "https://github.com/tamara-solution/opencart/blob/v20x/README.md";
    ...
    protected function validate() {
        ...
        if ($check_credentials) {
            $url = $this->model_payment_tamarapay->getApiUrl($this->request->post['tamarapay_api_environment']);
            ...
            if ($this->isChangedConfig('tamarapay_api_environment') || $this->isChangedConfig('tamarapay_token')) {
                try {
                    $client = $this->model_payment_tamarapay->createClient(['url' => $url, 'token' => $token]);

                    /**
                     * @var $response \TMS\Tamara\Response\Checkout\GetPaymentTypesResponse
                     */
                    $response = $this->model_payment_tamarapay->getPaymentTypesOfClient($client);
    ...
    private function validateWebhook() {
        ...
        if (!empty($webhookEnabled)) {
            if (empty($webhookId)) {
                try {
                    $webhookId = $this->model_payment_tamarapay->registerWebhook($this->model_payment_tamarapay->getApiUrl($this->request->post['tamarapay_api_environment']),
    ...
    private function removeLegacyAutoload() {
        ...
        $firstOriginalDir = realpath(DIR_SYSTEM . '../tamarapay');
        if (!empty($firstOriginalDir)) {
            $this->deleteDir($firstOriginalDir);
        }
        $secondOriginalDir = realpath(DIR_SYSTEM . '../tamara');
        if (!empty($secondOriginalDir)) {
            $this->deleteDir($secondOriginalDir);
        }
        return true;
    }
    ...
    private function addEventToShowPromoWidgetOnCartPage() {
        ...
        $this->model_extension_event->addEvent('tamara_promo_wg_cart', 'catalog/view/*/template/checkout/cart/after', 'payment/tamarapay/addPromoWidgetForCartPage');
    }
...
//                $this->addEventToShowPromoWidgetOnCartPage();
    ...
    private function moveApiUrlConfig() {
        ...
        if ($url == $this->model_payment_tamarapay->getProductionApiUrl()) {
            $apiEnvironment = $this->model_payment_tamarapay->getProductionApiEnvironment();
        } else {
            $apiEnvironment = $this->model_payment_tamarapay->getSandboxApiEnvironment();
        }
        $this->model_payment_tamarapay->saveConfig("tamarapay_api_environment", $apiEnvironment);
    ...
    public function flushTamaraCache() {
        $this->load->model('payment/tamarapay');
        $this->model_payment_tamarapay->removePaymentTypesCache();
    ...
    private function getTamaraPaymentUrlFromConfig() {
        $this->load->model('payment/tamarapay');
        return $this->model_payment_tamarapay->getApiUrl();
    }
