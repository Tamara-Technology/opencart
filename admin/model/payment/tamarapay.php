...
class ModelPaymentTamarapay extends Model
{
    ...
    private function copyTemplates() {
        ...
        $defaultThemePaymentDir = $themeDir . sprintf("default%stemplate%spayment%s", DIRECTORY_SEPARATOR, DIRECTORY_SEPARATOR, DIRECTORY_SEPARATOR);
        $needToCopies = ["tamarapay.tpl", "tamarapay_success.tpl"];
        foreach ($themes as $theme) {
            if ($this->endsWith($theme, "default")) {
                continue;
            }
            $themePaymentDir = $theme . sprintf("%stemplate%spayment%s", DIRECTORY_SEPARATOR, DIRECTORY_SEPARATOR, DIRECTORY_SEPARATOR);
