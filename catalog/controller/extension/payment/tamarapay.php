...
class ControllerExtensionPaymentTamarapay extends Controller
{
    ...
    public function index()
    {
        ...
        $data['language_code'] = $this->model_extension_payment_tamarapay->getLanguageCodeFromSession();
        $data['country_code'] = $this->model_extension_payment_tamarapay->getSupportedCountriesByCurrency($this->model_extension_payment_tamarapay->getCurrencyCodeFromSession())[0];
    ...
    public function send()
    {
        try {
            ...
            $phoneNumber = $this->model_extension_payment_tamarapay->getCustomerPhoneNumberFromSession();
    ...
    private function validatePaymentTypeSubmitted($name, $countryCode, $orderValue, $phoneNumber) {
        $paymentOptionsAvailability = $this->model_extension_payment_tamarapay->checkPaymentOptionsAvailability($countryCode, $orderValue, $phoneNumber);
...
//            $this->model_checkout_order->addOrderHistory($data['order_id'], $successStatusId, "Tamara - Pay success", false);
    ...
    public function addPromoWidgetForProduct($route, $data, $output) {
        $this->load->model('extension/payment/tamarapay');
        if (!$this->model_extension_payment_tamarapay->isTamaraEnabled()) {
            return $output;
        }
        if ($this->model_extension_payment_tamarapay->getDisableTamara()) {
            return $output;
        }
    ...
    public function getWidgetHtml($price) {
        ...
        $languageCode = $this->model_extension_payment_tamarapay->getLanguageCodeFromSession();
        $publicKey = $this->model_extension_payment_tamarapay->getMerchantPublicKey();
