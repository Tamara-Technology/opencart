...
class ModelPaymentTamarapay extends Model
{
    ...
    public function getMerchantUrls()
    {
        ...
        $successUrl = $baseUrl . 'index.php?route=payment/tamarapay/success';
        $failureUrl = $baseUrl . 'index.php?route=payment/tamarapay/failure';
        $cancelUrl = $baseUrl . 'index.php?route=payment/tamarapay/cancel';
        $notificationUrl = $baseUrl . 'index.php?route=payment/tamarapay/notification';
        $result = [
            'success' => $successUrl,
            'failure' => $failureUrl,
            'cancel' => $cancelUrl,
    ...
    public function log($data, $class_step = 1, $function_step = 1)
    {
        if ($this->config->get('tamarapay_debug')) {
            ...
            if (isset($backtrace[$class_step]) && isset($backtrace[$function_step])) {
