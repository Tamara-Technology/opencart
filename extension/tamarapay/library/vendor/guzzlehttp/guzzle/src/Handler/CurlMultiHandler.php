...
use TMS\GuzzleHttp\Promise\Promise;
...
class CurlMultiHandler
{
    ...
    {
        $this->factory = $options['handle_factory'] ?? new \TMS\GuzzleHttp\Handler\CurlFactory(50);
        if (isset($options['select_timeout'])) {
            $this->selectTimeout = $options['select_timeout'];
        } elseif ($selectTimeout = \TMS\GuzzleHttp\Utils::getenv('GUZZLE_CURL_SELECT_TIMEOUT')) {
    ...
    public function __invoke(\TMS\Psr\Http\Message\RequestInterface $request, array $options) : \TMS\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        $id = (int) $easy->handle;
        $promise = new \TMS\GuzzleHttp\Promise\Promise([$this, 'execute'], function () use($id) {
    ...
    public function tick() : void
    {
        ...
        if ($this->delays) {
            $currentTime = \TMS\GuzzleHttp\Utils::currentTime();
    ...
    {
        $queue = \TMS\GuzzleHttp\Promise\Utils::queue();
    ...
    private function addRequest(array $entry) : void
    {
        ...
        if (empty($easy->options['delay'])) {
            ...
            $this->delays[$id] = \TMS\GuzzleHttp\Utils::currentTime() + $easy->options['delay'] / 1000;
        }
    }
    /**
    ...
    private function processMessages() : void
    {
        while ($done = \curl_multi_info_read($this->_mh)) {
            ...
            $entry['deferred']->resolve(\TMS\GuzzleHttp\Handler\CurlFactory::finish($this, $entry['easy'], $this->factory));
        }
    }
