 ...
 * @final
 */
class MessageFormatter implements \TMS\GuzzleHttp\MessageFormatterInterface
{
    /**
    ...
    public function format(\TMS\Psr\Http\Message\RequestInterface $request, ?\TMS\Psr\Http\Message\ResponseInterface $response = null, ?\Throwable $error = null) : string
    {
        ...
        return \preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \TMS\GuzzleHttp\Psr7\Message::toString($request);
                    break;
                case 'response':
                    $result = $response ? \TMS\GuzzleHttp\Psr7\Message::toString($response) : '';
                    break;
                case 'req_headers':
                ...
                case 'req_body':
                    ...
                    break;
                case 'res_body':
                    if (!$response instanceof \TMS\Psr\Http\Message\ResponseInterface) {
                        $result = 'NULL';
                        break;
                    }
     ...
     */
    private function headers(\TMS\Psr\Http\Message\MessageInterface $message) : string
    {
        $result = '';
