    ...
    public function __invoke(\TMS\Psr\Http\Message\RequestInterface $request, array $options) : \TMS\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if (\is_string($uri) && ($type = \TMS\GuzzleHttp\Psr7\MimeType::fromFilename($uri))) {
        ...
        return $fn(\TMS\GuzzleHttp\Psr7\Utils::modifyRequest($request, $modify), $options);
    }
    /**
     * Add expect header
     */
    private function addExpectHeader(\TMS\Psr\Http\Message\RequestInterface $request, array $options, array &$modify) : void
    {
