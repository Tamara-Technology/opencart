...
class PrepareBodyMiddleware
{
    ...
    public function __invoke(\TMS\Psr\Http\Message\RequestInterface $request, array $options)
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if ($type = \TMS\GuzzleHttp\Psr7\mimetype_from_filename($uri)) {
        ...
        return $fn(\TMS\GuzzleHttp\Psr7\modify_request($request, $modify), $options);
    }
    /**
     * Add expect header
     *
     * @return void
     */
    private function addExpectHeader(\TMS\Psr\Http\Message\RequestInterface $request, array $options, array &$modify)
    {
