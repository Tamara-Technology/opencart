...
namespace TMS\Firebase\JWT;

use ArrayAccess;
use DomainException;
use Exception;
use InvalidArgumentException;
use TMS\OpenSSLAsymmetricKey;
...
class JWT
{
    ...
    public static function decode($jwt, $keyOrKeyArray, array $allowed_algs = array())
    {
        ...
        if (!static::verify("{$headb64}.{$bodyb64}", $sig, $keyMaterial, $header->alg)) {
            throw new \TMS\Firebase\JWT\SignatureInvalidException('Signature verification failed');
        }
        // Check the nbf if it is defined. This is the time that the
        // token can actually be used. If it's not yet that time, abort.
        if (isset($payload->nbf) && $payload->nbf > $timestamp + static::$leeway) {
            throw new \TMS\Firebase\JWT\BeforeValidException('Cannot handle token prior to ' . \date(\DateTime::ISO8601, $payload->nbf));
        }
        // Check that this token has been created before 'now'. This prevents
        // using tokens that have been created for later use (and haven't
        // correctly used the nbf claim).
        if (isset($payload->iat) && $payload->iat > $timestamp + static::$leeway) {
            throw new \TMS\Firebase\JWT\BeforeValidException('Cannot handle token prior to ' . \date(\DateTime::ISO8601, $payload->iat));
        }
        // Check if this token has expired.
        if (isset($payload->exp) && $timestamp - static::$leeway >= $payload->exp) {
            throw new \TMS\Firebase\JWT\ExpiredException('Expired token');
        }
        return $payload;
    }
    /**
    ...
    public static function sign($msg, $key, $alg = 'HS256')
    {
        ...
        switch ($function) {
            ...
            case 'openssl':
                ...
                return $signature;
            case 'sodium_crypto':
                if (!\function_exists('TMS\\sodium_crypto_sign_detached')) {
    ...
    private static function verify($msg, $signature, $key, $alg)
    {
        ...
        switch ($function) {
            ...
            case 'sodium_crypto':
                if (!\function_exists('TMS\\sodium_crypto_sign_verify_detached')) {
                    throw new \DomainException('libsodium is not available');
                }
                try {
                    // The last non-empty line is used as the key.
    ...
    {
        if (\is_string($keyOrKeyArray) || \is_resource($keyOrKeyArray) || $keyOrKeyArray instanceof \TMS\OpenSSLAsymmetricKey) {
            return array($keyOrKeyArray, null);
        }
        if ($keyOrKeyArray instanceof \TMS\Firebase\JWT\Key) {
        ...
        if (\is_array($keyOrKeyArray) || $keyOrKeyArray instanceof \ArrayAccess) {
            ...
            if ($key instanceof \TMS\Firebase\JWT\Key) {
