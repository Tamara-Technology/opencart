...
class RequestDispatcher
{
    ...
    public function dispatch($request)
    {
        ...
        if ($response->getStatusCode() == 401) {
            ...
            $this->getOcDbInstance()->query("UPDATE `" . DB_PREFIX . "setting` SET `value` = '0' WHERE `code` = 'payment_tamarapay' AND `key` = 'payment_tamarapay_status'");
        }
        return $response;
    }
    ...
    public function getOcDbInstance() {
        ...
        $ocDb = new \DB($config->get('db_engine'), $config->get('db_hostname'), $config->get('db_username'), $config->get('db_password'), $config->get('db_database'), $config->get('db_port'));
        return $ocDb;
    }
