...
namespace TMS\Buzz\Message;

use TMS\Buzz\Exception\InvalidArgumentException;
use TMS\Http\Message\ResponseFactory as HTTPlugResponseFactory;
use TMS\Psr\Http\Message\ResponseFactoryInterface as PsrResponseFactory;
...
final class ResponseBuilder
{
    ...
    {
        if (!$responseFactory instanceof \TMS\Http\Message\ResponseFactory && !$responseFactory instanceof \TMS\Psr\Http\Message\ResponseFactoryInterface) {
            throw new \TMS\Buzz\Exception\InvalidArgumentException('First parameter to ResponseBuilder must be a response factory');
        }
        $this->responseFactory = $responseFactory;
        $this->response = $responseFactory->createResponse();
    }
    public function getResponseFromRawInput(string $raw, int $headerSize) : \TMS\Psr\Http\Message\ResponseInterface
    {
    ...
    public function setStatus(string $input) : void
    {
        ...
        if (\count($parts) < 2 || 0 !== \strpos(\strtolower($parts[0]), 'http/')) {
            throw new \TMS\Buzz\Exception\InvalidArgumentException(\sprintf('"%s" is not a valid HTTP status line', $input));
        }
    ...
    public function parseHttpHeaders(array $headers) : void
    {
        ...
        } catch (\TMS\Buzz\Exception\InvalidArgumentException $e) {
