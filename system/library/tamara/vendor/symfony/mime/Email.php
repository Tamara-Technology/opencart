 ...
 */
class Email extends \TMS\Symfony\Component\Mime\Message
{
    ...
    {
        return $this->setHeaderBody('Path', 'Return-Path', \TMS\Symfony\Component\Mime\Address::create($address));
    }
    public function getReturnPath() : ?\TMS\Symfony\Component\Mime\Address
    {
    ...
    {
        return $this->setHeaderBody('Mailbox', 'Sender', \TMS\Symfony\Component\Mime\Address::create($address));
    }
    public function getSender() : ?\TMS\Symfony\Component\Mime\Address
    {
     ...
     * @return $this
     */
    public function attachPart(\TMS\Symfony\Component\Mime\Part\DataPart $part)
    {
        $this->cachedBody = null;
    ...
    public function ensureValidity()
    {
        if (null === $this->text && null === $this->html && !$this->attachments) {
            throw new \TMS\Symfony\Component\Mime\Exception\LogicException('A message must have a text or an HTML part or attachments.');
        }
     ...
     */
    private function generateBody() : \TMS\Symfony\Component\Mime\Part\AbstractPart
    {
        ...
        $part = null === $this->text ? null : new \TMS\Symfony\Component\Mime\Part\TextPart($this->text, $this->textCharset);
        if (null !== $htmlPart) {
            if (null !== $part) {
                $part = new \TMS\Symfony\Component\Mime\Part\Multipart\AlternativePart($part, $htmlPart);
            } else {
                $part = $htmlPart;
            }
        }
        if ($relatedParts) {
            $part = new \TMS\Symfony\Component\Mime\Part\Multipart\RelatedPart($part, ...$relatedParts);
        }
        if ($otherParts) {
            if ($part) {
                $part = new \TMS\Symfony\Component\Mime\Part\Multipart\MixedPart($part, ...$otherParts);
            } else {
                $part = new \TMS\Symfony\Component\Mime\Part\Multipart\MixedPart(...$otherParts);
            }
        }
    ...
    private function prepareParts() : ?array
    {
        ...
        if (null !== $html) {
            ...
            $htmlPart = new \TMS\Symfony\Component\Mime\Part\TextPart($html, $this->htmlCharset, 'html');
        ...
        $nameRef = new \ReflectionProperty(\TMS\Symfony\Component\Mime\Part\TextPart::class, 'name');
    ...
    private function createDataPart(array $attachment) : \TMS\Symfony\Component\Mime\Part\DataPart
    {
        if (isset($attachment['part'])) {
            return $attachment['part'];
        }
        if (isset($attachment['body'])) {
            $part = new \TMS\Symfony\Component\Mime\Part\DataPart($attachment['body'], $attachment['name'] ?? null, $attachment['content-type'] ?? null);
        } else {
            $part = \TMS\Symfony\Component\Mime\Part\DataPart::fromPath($attachment['path'] ?? '', $attachment['name'] ?? null, $attachment['content-type'] ?? null);
        }
    ...
    private function addListAddressHeaderBody(string $name, array $addresses)
    {
        ...
        $header->addAddresses(\TMS\Symfony\Component\Mime\Address::createArray($addresses));
        return $this;
    }
    private function setListAddressHeaderBody(string $name, array $addresses)
    {
        $addresses = \TMS\Symfony\Component\Mime\Address::createArray($addresses);
