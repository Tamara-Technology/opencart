 ...
 */
class DataPart extends \TMS\Symfony\Component\Mime\Part\TextPart
{
    /** @internal */
    protected $_parent;
    ...
    public static function fromPath(string $path, string $name = null, string $contentType = null) : self
    {
        if (null === $contentType) {
            ...
            if (null === self::$mimeTypes) {
                self::$mimeTypes = new \TMS\Symfony\Component\Mime\MimeTypes();
            }
            $contentType = self::$mimeTypes->getMimeTypes($ext)[0] ?? 'application/octet-stream';
        }
        if (\is_file($path) && !\is_readable($path) || \is_dir($path)) {
            throw new \TMS\Symfony\Component\Mime\Exception\InvalidArgumentException(\sprintf('Path "%s" is not readable.', $path));
        }
        if (\false === ($handle = @\fopen($path, 'r', \false))) {
            throw new \TMS\Symfony\Component\Mime\Exception\InvalidArgumentException(\sprintf('Unable to open path "%s".', $path));
        }
        if (!\is_file($path)) {
    ...
    public function __sleep()
    {
        ...
        foreach (['body', 'charset', 'subtype', 'disposition', 'name', 'encoding'] as $name) {
            $r = new \ReflectionProperty(\TMS\Symfony\Component\Mime\Part\TextPart::class, $name);
            $r->setAccessible(\true);
