...
use TMS\Buzz\Exception\CallbackException;
use TMS\Buzz\Exception\ClientException;
use TMS\Buzz\Exception\NetworkException;
use TMS\Buzz\Exception\RequestException;
 ...
 */
abstract class AbstractCurl extends \TMS\Buzz\Client\AbstractClient
{
    private $handles = [];
    ...
    protected function createHandle()
    {
        ...
        if (\false === $curl) {
            throw new \TMS\Buzz\Exception\ClientException('Unable to create a new cURL handle');
        }
        return $curl;
    }
    /**
     ...
     * @param resource $curl
     */
    protected function prepare($curl, \TMS\Psr\Http\Message\RequestInterface $request, \TMS\Buzz\Configuration\ParameterBag $options) : \TMS\Buzz\Message\ResponseBuilder
    {
        ...
        $responseBuilder = new \TMS\Buzz\Message\ResponseBuilder($this->responseFactory);
     ...
     */
    private function setOptionsFromRequest($curl, \TMS\Psr\Http\Message\RequestInterface $request) : void
    {
        $options = [\CURLOPT_CUSTOMREQUEST => $request->getMethod(), \CURLOPT_URL => $request->getUri()->__toString(), \CURLOPT_HTTPHEADER => \TMS\Buzz\Message\HeaderConverter::toBuzzHeaders($request->getHeaders())];
     ...
     * @param resource $curl
     */
    private function setOptionsFromParameterBag($curl, \TMS\Buzz\Configuration\ParameterBag $options) : void
    {
     ...
     */
    protected function parseError(\TMS\Psr\Http\Message\RequestInterface $request, int $errno, $curl) : void
    {
        switch ($errno) {
            case \CURLE_OK:
            ...
            case \CURLE_SSL_CONNECT_ERROR:
                throw new \TMS\Buzz\Exception\NetworkException($request, \curl_error($curl), $errno);
            case \CURLE_ABORTED_BY_CALLBACK:
                throw new \TMS\Buzz\Exception\CallbackException($request, \curl_error($curl), $errno);
            default:
                throw new \TMS\Buzz\Exception\RequestException($request, \curl_error($curl), $errno);
        }
    }
    private function getProtocolVersion(\TMS\Psr\Http\Message\RequestInterface $request) : int
    {
