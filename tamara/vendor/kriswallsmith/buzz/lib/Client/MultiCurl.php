...
class MultiCurl extends \TMS\Buzz\Client\AbstractCurl implements \TMS\Buzz\Client\BatchClientInterface, \TMS\Buzz\Client\BuzzClientInterface
{
    private $queue = [];
    private $curlm;
    /**
     ...
     *     };
     */
    public function sendAsyncRequest(\TMS\Psr\Http\Message\RequestInterface $request, array $options = []) : void
    {
    ...
    public function sendRequest(\TMS\Psr\Http\Message\RequestInterface $request, array $options = []) : \TMS\Psr\Http\Message\ResponseInterface
    {
        ...
        $options = $options->add(['callback' => function (\TMS\Psr\Http\Message\RequestInterface $request, \TMS\Psr\Http\Message\ResponseInterface $response = null, \TMS\Buzz\Exception\ClientException $e = null) use(&$responseToReturn, $originalCallback) {
    ...
    protected function configureOptions(\TMS\Symfony\Component\OptionsResolver\OptionsResolver $resolver) : void
    {
        ...
        $resolver->setDefault('callback', function (\TMS\Psr\Http\Message\RequestInterface $request, \TMS\Psr\Http\Message\ResponseInterface $response = null, \TMS\Buzz\Exception\ClientException $e = null) {
        });
    ...
    private function addToQueue(\TMS\Psr\Http\Message\RequestInterface $request, \TMS\Buzz\Configuration\ParameterBag $options) : array
    {
    ...
    private function initMultiCurlHandle() : void
    {
        ...
        if (\false === $this->curlm) {
            throw new \TMS\Buzz\Exception\ClientException('Unable to create a new cURL multi handle');
        }
    ...
    private function initQueue() : void
    {
        foreach ($this->queue as $i => $queueItem) {
            ...
            if ($this->serverPushSupported && $options->get('use_pushed_response') && $this->hasPushResponse($request->getUri()->__toString())) {
                ...
                $response = (new \TMS\Buzz\Message\ResponseBuilder($this->responseFactory))->getResponseFromRawInput($data['content'], $data['headerSize']);
