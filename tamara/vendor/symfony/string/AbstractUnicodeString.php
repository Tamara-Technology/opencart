 ...
 */
abstract class AbstractUnicodeString extends \TMS\Symfony\Component\String\AbstractString
{
                ...
                if ('nfd' === ($rule = \strtolower($rule))) {
                    ...
                    if (null === ($transliterator = self::$transliterators[$rule] ?? (self::$transliterators[$rule] = \Transliterator::create($rule)))) {
                        ...
                        if (null === $transliterator) {
                            throw new \TMS\Symfony\Component\String\Exception\InvalidArgumentException(\sprintf('Unknown transliteration rule "%s".', $rule));
                        }
    ...
    public function folded(bool $compat = \true) : \TMS\parent
    {
        $str = clone $this;
    ...
    public function join(array $strings, string $lastGlue = null) : \TMS\parent
    {
        ...
        if (!\preg_match('//u', $str->string)) {
            throw new \TMS\Symfony\Component\String\Exception\InvalidArgumentException('Invalid UTF-8 string.');
        }
        return $str;
    }
    ...
    public function normalize(int $form = self::NFC) : self
    {
        if (!\in_array($form, [self::NFC, self::NFD, self::NFKC, self::NFKD])) {
            throw new \TMS\Symfony\Component\String\Exception\InvalidArgumentException('Unsupported normalization form.');
        }
        $str = clone $this;
        ...
        if (\is_array($to) || $to instanceof \Closure) {
            ...
            $to = static function (array $m) use($to) : string {
                ...
                if ('' !== $to && (!\is_string($to) || !\preg_match('//u', $to))) {
                    throw new \TMS\Symfony\Component\String\Exception\InvalidArgumentException('Replace callback must return a valid UTF-8 string.');
                }
                return $to;
            };
    ...
    public function snake() : \TMS\parent
    {
        ...
        $str->string = \mb_strtolower(\preg_replace(['/(\\p{Lu}+)(\\p{Lu}\\p{Ll})/u', '/([\\p{Ll}0-9])(\\p{Lu})/u'], 'TMS\\1_\\2', $str->string), 'UTF-8');
        return $str;
    }
    ...
    public function trim(string $chars = " \t\n\r\x00\v\f ﻿") : \TMS\parent
    {
        if (" \t\n\r\x00\v\f ﻿" !== $chars && !\preg_match('//u', $chars)) {
            throw new \TMS\Symfony\Component\String\Exception\InvalidArgumentException('Invalid UTF-8 chars.');
        }
     ...
     * @return static
     */
    private function pad(int $len, self $pad, int $type) : \TMS\parent
    {
        $sLen = $this->length();
        if ($len <= $sLen) {
            return clone $this;
        ...
        switch ($type) {
            ...
            default:
                throw new \TMS\Symfony\Component\String\Exception\InvalidArgumentException('Invalid padding type.');
        }
    }
    /**
