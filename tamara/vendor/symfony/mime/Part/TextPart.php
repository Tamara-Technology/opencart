 ...
 */
namespace TMS\Symfony\Component\Mime\Part;

use TMS\Symfony\Component\Mime\Encoder\Base64ContentEncoder;
use TMS\Symfony\Component\Mime\Encoder\ContentEncoderInterface;
use TMS\Symfony\Component\Mime\Encoder\EightBitContentEncoder;
use TMS\Symfony\Component\Mime\Encoder\QpContentEncoder;
use TMS\Symfony\Component\Mime\Exception\InvalidArgumentException;
use TMS\Symfony\Component\Mime\Header\Headers;
/**
 * @author Fabien Potencier <fabien@symfony.com>
 */
class TextPart extends \TMS\Symfony\Component\Mime\Part\AbstractPart
{
    ...
    public function __construct($body, ?string $charset = 'utf-8', string $subtype = 'plain', string $encoding = null)
    {
        ...
        if (null === $encoding) {
            ...
            if ('quoted-printable' !== $encoding && 'base64' !== $encoding && '8bit' !== $encoding) {
                throw new \TMS\Symfony\Component\Mime\Exception\InvalidArgumentException(\sprintf('The encoding must be one of "quoted-printable", "base64", or "8bit" ("%s" given).', $encoding));
            }
    ...
    private function getEncoder() : \TMS\Symfony\Component\Mime\Encoder\ContentEncoderInterface
    {
        if ('8bit' === $this->encoding) {
            return self::$encoders[$this->encoding] ?? (self::$encoders[$this->encoding] = new \TMS\Symfony\Component\Mime\Encoder\EightBitContentEncoder());
        }
        if ('quoted-printable' === $this->encoding) {
            return self::$encoders[$this->encoding] ?? (self::$encoders[$this->encoding] = new \TMS\Symfony\Component\Mime\Encoder\QpContentEncoder());
        }
        return self::$encoders[$this->encoding] ?? (self::$encoders[$this->encoding] = new \TMS\Symfony\Component\Mime\Encoder\Base64ContentEncoder());
    }
    ...
    {
        $r = new \ReflectionProperty(\TMS\Symfony\Component\Mime\Part\AbstractPart::class, 'headers');
        $r->setAccessible(\true);
