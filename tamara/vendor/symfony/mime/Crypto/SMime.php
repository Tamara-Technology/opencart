...
use TMS\Symfony\Component\Mime\Part\SMimePart;
/**
...
abstract class SMime
{
    ...
    {
        if (!\file_exists($path)) {
            throw new \TMS\Symfony\Component\Mime\Exception\RuntimeException(\sprintf('File does not exist: "%s".', $path));
        }
    ...
    protected function convertMessageToSMimePart($stream, string $type, string $subtype) : \TMS\Symfony\Component\Mime\Part\SMimePart
    {
        \rewind($stream);
        $headers = '';
        while (!\feof($stream)) {
        ...
        return new \TMS\Symfony\Component\Mime\Part\SMimePart($this->getStreamIterator($stream), $type, $subtype, $this->getParametersFromHeader($headers['content-type']));
    }
