 ...
 */
final class SMimeSigner extends \TMS\Symfony\Component\Mime\Crypto\SMime
{
    ...
    public function sign(\TMS\Symfony\Component\Mime\Message $message) : \TMS\Symfony\Component\Mime\Message
    {
        $bufferFile = \tmpfile();
        $outputFile = \tmpfile();
        ...
        if (!@\openssl_pkcs7_sign(\stream_get_meta_data($bufferFile)['uri'], \stream_get_meta_data($outputFile)['uri'], $this->signCertificate, $this->signPrivateKey, [], $this->signOptions, $this->extraCerts)) {
            throw new \TMS\Symfony\Component\Mime\Exception\RuntimeException(\sprintf('Failed to sign S/Mime message. Error: "%s".', \openssl_error_string()));
        }
        return new \TMS\Symfony\Component\Mime\Message($message->getHeaders(), $this->convertMessageToSMimePart($outputFile, 'multipart', 'signed'));
    }
}
