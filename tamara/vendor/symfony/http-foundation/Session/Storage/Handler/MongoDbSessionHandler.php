 ...
 */
class MongoDbSessionHandler extends \TMS\Symfony\Component\HttpFoundation\Session\Storage\Handler\AbstractSessionHandler
{
    private $mongo;
    /**
     ...
     */
    public function __construct(\TMS\MongoDB\Client $mongo, array $options)
    {
    ...
    {
        $expiry = new \TMS\MongoDB\BSON\UTCDateTime((\time() + (int) \ini_get('session.gc_maxlifetime')) * 1000);
        $this->getCollection()->updateOne([$this->options['id_field'] => $sessionId], ['$set' => [$this->options['time_field'] => new \TMS\MongoDB\BSON\UTCDateTime(), $this->options['expiry_field'] => $expiry]]);
        return \true;
    }
    /**
     * {@inheritdoc}
    ...
    private function getCollection() : \TMS\MongoDB\Collection
    {
