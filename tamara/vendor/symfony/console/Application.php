...
use TMS\Symfony\Component\Console\Helper\Helper;
use TMS\Symfony\Component\Console\Helper\HelperSet;
use TMS\Symfony\Component\Console\Helper\ProcessHelper;
use TMS\Symfony\Component\Console\Helper\QuestionHelper;
use TMS\Symfony\Component\Console\Input\ArgvInput;
use TMS\Symfony\Component\Console\Input\ArrayInput;
use TMS\Symfony\Component\Console\Input\InputArgument;
use TMS\Symfony\Component\Console\Input\InputAwareInterface;
 ...
 */
class Application implements \TMS\Symfony\Contracts\Service\ResetInterface
{
    private $commands = [];
     ...
     */
    public function run(\TMS\Symfony\Component\Console\Input\InputInterface $input = null, \TMS\Symfony\Component\Console\Output\OutputInterface $output = null)
    {
        ...
        if (null === $input) {
            $input = new \TMS\Symfony\Component\Console\Input\ArgvInput();
        }
        if (null === $output) {
            $output = new \TMS\Symfony\Component\Console\Output\ConsoleOutput();
        }
     ...
     * @return int 0 if everything went fine, or an error code
     */
    public function doRun(\TMS\Symfony\Component\Console\Input\InputInterface $input, \TMS\Symfony\Component\Console\Output\OutputInterface $output)
    {
        ...
        } catch (\TMS\Symfony\Component\Console\Exception\ExceptionInterface $e) {
        ...
        if (\true === $input->hasParameterOption(['--help', '-h'], \true)) {
            if (!$name) {
                $name = 'help';
                $input = new \TMS\Symfony\Component\Console\Input\ArrayInput(['command_name' => $this->defaultCommand]);
            } else {
                $this->wantHelps = \true;
            }
        }
        if (!$name) {
            ...
            $definition->setArguments(\array_merge($definition->getArguments(), ['command' => new \TMS\Symfony\Component\Console\Input\InputArgument('command', \TMS\Symfony\Component\Console\Input\InputArgument::OPTIONAL, $definition->getArgument('command')->getDescription(), $name)]));
        }
        try {
            ...
            if (!($e instanceof \TMS\Symfony\Component\Console\Exception\CommandNotFoundException && !$e instanceof \TMS\Symfony\Component\Console\Exception\NamespaceNotFoundException) || 1 !== \count($alternatives = $e->getAlternatives()) || !$input->isInteractive()) {
                if (null !== $this->dispatcher) {
                    $event = new \TMS\Symfony\Component\Console\Event\ConsoleErrorEvent($input, $output, $e);
                    $this->dispatcher->dispatch($event, \TMS\Symfony\Component\Console\ConsoleEvents::ERROR);
            ...
            $style = new \TMS\Symfony\Component\Console\Style\SymfonyStyle($input, $output);
    ...
    public function setHelperSet(\TMS\Symfony\Component\Console\Helper\HelperSet $helperSet)
    {
    ...
    public function setDefinition(\TMS\Symfony\Component\Console\Input\InputDefinition $definition)
    {
     ...
     */
    public function add(\TMS\Symfony\Component\Console\Command\Command $command)
    {
        $this->init();
    ...
    public function get($name)
    {
        ...
        if (!isset($this->commands[$name])) {
            throw new \TMS\Symfony\Component\Console\Exception\CommandNotFoundException(\sprintf('The "%s" command cannot be found because it is registered under multiple names. Make sure you don\'t set a different name via constructor or "setName()".', $name));
        }
    ...
    public function findNamespace($namespace)
    {
        ...
        if (empty($namespaces)) {
            ...
            throw new \TMS\Symfony\Component\Console\Exception\NamespaceNotFoundException($message, $alternatives);
        }
        $exact = \in_array($namespace, $namespaces, \true);
        if (\count($namespaces) > 1 && !$exact) {
            throw new \TMS\Symfony\Component\Console\Exception\NamespaceNotFoundException(\sprintf("The namespace \"%s\" is ambiguous.\nDid you mean one of these?\n%s.", $namespace, $this->getAbbreviationSuggestions(\array_values($namespaces))), \array_values($namespaces));
        }
    ...
    public function find($name)
    {
        ...
        if (empty($commands) || \count(\preg_grep('{^' . $expr . '$}i', $commands)) < 1) {
            ...
            throw new \TMS\Symfony\Component\Console\Exception\CommandNotFoundException($message, \array_values($alternatives));
        }
        // filter out aliases for commands which are already on the list
        if (\count($commands) > 1) {
            $commandList = $this->commandLoader ? \array_merge(\array_flip($this->commandLoader->getNames()), $this->commands) : $this->commands;
            $commands = \array_unique(\array_filter($commands, function ($nameOrAlias) use(&$commandList, $commands, &$aliases) {
                if (!$commandList[$nameOrAlias] instanceof \TMS\Symfony\Component\Console\Command\Command) {
        ...
        if (\count($commands) > 1) {
            ...
            foreach ($abbrevs as $abbrev) {
                $maxLen = \max(\TMS\Symfony\Component\Console\Helper\Helper::strlen($abbrev), $maxLen);
            }
            $abbrevs = \array_map(function ($cmd) use($commandList, $usableWidth, $maxLen, &$commands) {
                ...
                return \TMS\Symfony\Component\Console\Helper\Helper::strlen($abbrev) > $usableWidth ? \TMS\Symfony\Component\Console\Helper\Helper::substr($abbrev, 0, $usableWidth - 3) . '...' : $abbrev;
            }, \array_values($commands));
            if (\count($commands) > 1) {
                $suggestions = $this->getAbbreviationSuggestions(\array_filter($abbrevs));
                throw new \TMS\Symfony\Component\Console\Exception\CommandNotFoundException(\sprintf("Command \"%s\" is ambiguous.\nDid you mean one of these?\n%s.", $name, $suggestions), \array_values($commands));
            }
        }
    ...
    private function doActuallyRenderThrowable(\Throwable $e, \TMS\Symfony\Component\Console\Output\OutputInterface $output) : void
    {
        do {
            ...
            if ('' === $message || \TMS\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERBOSE <= $output->getVerbosity()) {
                $class = get_debug_type($e);
                $title = \sprintf('  [%s%s]  ', $class, 0 !== ($code = $e->getCode()) ? ' (' . $code . ')' : '');
                $len = \TMS\Symfony\Component\Console\Helper\Helper::strlen($title);
            } else {
                $len = 0;
            }
            ...
            foreach ('' !== $message ? \preg_split('/\\r?\\n/', $message) : [] as $line) {
                foreach ($this->splitStringByWidth($line, $width - 4) as $line) {
                    ...
                    $lineLength = \TMS\Symfony\Component\Console\Helper\Helper::strlen($line) + 4;
                    $lines[] = [$line, $lineLength];
                    $len = \max($lineLength, $len);
                }
            }
            $messages = [];
            if (!$e instanceof \TMS\Symfony\Component\Console\Exception\ExceptionInterface || \TMS\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERBOSE <= $output->getVerbosity()) {
                $messages[] = \sprintf('<comment>%s</comment>', \TMS\Symfony\Component\Console\Formatter\OutputFormatter::escape(\sprintf('In %s line %s:', \basename($e->getFile()) ?: 'n/a', $e->getLine() ?: 'n/a')));
            }
            $messages[] = $emptyLine = \sprintf('<error>%s</error>', \str_repeat(' ', $len));
            if ('' === $message || \TMS\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERBOSE <= $output->getVerbosity()) {
                $messages[] = \sprintf('<error>%s%s</error>', $title, \str_repeat(' ', \max(0, $len - \TMS\Symfony\Component\Console\Helper\Helper::strlen($title))));
            }
            foreach ($lines as $line) {
                $messages[] = \sprintf('<error>  %s  %s</error>', \TMS\Symfony\Component\Console\Formatter\OutputFormatter::escape($line[0]), \str_repeat(' ', $len - $line[1]));
            }
            $messages[] = $emptyLine;
            $messages[] = '';
            $output->writeln($messages, \TMS\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
            if (\TMS\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERBOSE <= $output->getVerbosity()) {
                $output->writeln('<comment>Exception trace:</comment>', \TMS\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
                ...
                for ($i = 0, $count = \count($trace); $i < $count; ++$i) {
                    ...
                    $output->writeln(\sprintf(' %s%s at <info>%s:%s</info>', $class, $function ? $type . $function . '()' : '', $file, $line), \TMS\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
                }
                $output->writeln('', \TMS\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
            }
     ...
     */
    protected function configureIO(\TMS\Symfony\Component\Console\Input\InputInterface $input, \TMS\Symfony\Component\Console\Output\OutputInterface $output)
    {
        ...
        switch ($shellVerbosity = (int) \getenv('SHELL_VERBOSITY')) {
            case -1:
                $output->setVerbosity(\TMS\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
                break;
            case 1:
                $output->setVerbosity(\TMS\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERBOSE);
                break;
            case 2:
                $output->setVerbosity(\TMS\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERY_VERBOSE);
                break;
            case 3:
                $output->setVerbosity(\TMS\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_DEBUG);
                break;
            default:
                $shellVerbosity = 0;
                break;
     ...
     * @return int 0 if everything went fine, or an error code
     */
    protected function doRunCommand(\TMS\Symfony\Component\Console\Command\Command $command, \TMS\Symfony\Component\Console\Input\InputInterface $input, \TMS\Symfony\Component\Console\Output\OutputInterface $output)
    {
        foreach ($command->getHelperSet() as $helper) {
            if ($helper instanceof \TMS\Symfony\Component\Console\Input\InputAwareInterface) {
        ...
        $event = new \TMS\Symfony\Component\Console\Event\ConsoleCommandEvent($command, $input, $output);
        $e = null;
        try {
            $this->dispatcher->dispatch($event, \TMS\Symfony\Component\Console\ConsoleEvents::COMMAND);
            if ($event->commandShouldRun()) {
                $exitCode = $command->run($input, $output);
            } else {
                $exitCode = \TMS\Symfony\Component\Console\Event\ConsoleCommandEvent::RETURN_CODE_DISABLED;
            }
        } catch (\Throwable $e) {
            $event = new \TMS\Symfony\Component\Console\Event\ConsoleErrorEvent($input, $output, $e, $command);
            $this->dispatcher->dispatch($event, \TMS\Symfony\Component\Console\ConsoleEvents::ERROR);
            $e = $event->getError();
        ...
        $event = new \TMS\Symfony\Component\Console\Event\ConsoleTerminateEvent($command, $input, $output, $exitCode);
        $this->dispatcher->dispatch($event, \TMS\Symfony\Component\Console\ConsoleEvents::TERMINATE);
        if (null !== $e) {
            throw $e;
        }
        return $event->getExitCode();
     ...
     * @return string|null
     */
    protected function getCommandName(\TMS\Symfony\Component\Console\Input\InputInterface $input)
    {
    ...
    {
        return [new \TMS\Symfony\Component\Console\Command\HelpCommand(), new \TMS\Symfony\Component\Console\Command\ListCommand()];
    }
    /**
    ...
    {
        return new \TMS\Symfony\Component\Console\Helper\HelperSet([new \TMS\Symfony\Component\Console\Helper\FormatterHelper(), new \TMS\Symfony\Component\Console\Helper\DebugFormatterHelper(), new \TMS\Symfony\Component\Console\Helper\ProcessHelper(), new \TMS\Symfony\Component\Console\Helper\QuestionHelper()]);
    }
    /**
