...
class MessageFormatter
{
     ...
     * @return string
     */
    public function format(\TMS\Psr\Http\Message\RequestInterface $request, \TMS\Psr\Http\Message\ResponseInterface $response = null, \Exception $error = null)
    {
        $cache = [];
        return \preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \TMS\GuzzleHttp\Psr7\str($request);
                    break;
                case 'response':
                    $result = $response ? \TMS\GuzzleHttp\Psr7\str($response) : '';
                    break;
                case 'req_headers':
     ...
     * @return string
     */
    private function headers(\TMS\Psr\Http\Message\MessageInterface $message)
    {
        $result = '';
