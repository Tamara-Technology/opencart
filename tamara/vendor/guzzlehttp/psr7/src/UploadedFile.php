...
class UploadedFile implements \TMS\Psr\Http\Message\UploadedFileInterface
{
    /**
     * @var int[]
     */
    ...
    private function setStreamOrFile($streamOrFile)
    {
        if (\is_string($streamOrFile)) {
            ...
            $this->stream = new \TMS\GuzzleHttp\Psr7\Stream($streamOrFile);
        } elseif ($streamOrFile instanceof \TMS\Psr\Http\Message\StreamInterface) {
    ...
    private function setError($error)
    {
        ...
        if (\false === \in_array($error, \TMS\GuzzleHttp\Psr7\UploadedFile::$errors)) {
    ...
    public function getStream()
    {
        ...
        return new \TMS\GuzzleHttp\Psr7\LazyOpenStream($this->file, 'r+');
    }
    /**
     * {@inheritdoc}
     *
    ...
    public function moveTo($targetPath)
    {
        ...
        if ($this->file) {
            ...
            \TMS\GuzzleHttp\Psr7\Utils::copyToStream($this->getStream(), new \TMS\GuzzleHttp\Psr7\LazyOpenStream($targetPath, 'w'));
            $this->moved = \true;
        }
        if (\false === $this->moved) {
